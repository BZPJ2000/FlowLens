 ...
 *
 * 来源: Proma-reusable-modules/05-code-highlight-service
 */
...
/**
 * 同步高亮代码，返回 HTML 字符串（高亮器未初始化时返回 null）
 */
export function highlightCodeSync(options: HighlightOptions): HighlightResult | null {
  if (!cachedHighlighter) return null
  const { code, language, theme = 'github-dark' } = options
  const resolved = resolveLanguage(language)
  const lang = resolved === 'text' ? 'text' : cachedHighlighter.getLoadedLanguages().includes(resolved) ? resolved : 'text'
